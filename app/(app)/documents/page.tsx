    ...
    <section className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Securely store and manage your personal and work files.</p>
        </div>
      ...
      <div>
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {[
               { name: "Contracts", items: 12, color: "text-blue-500", bg: "bg-blue-500/10" },
               { name: "Tax Forms", items: 4, color: "text-emerald-500", bg: "bg-emerald-500/10" },
               { name: "Personal", items: 28, color: "text-purple-500", bg: "bg-purple-500/10" },
               { name: "Policies", items: 8, color: "text-amber-500", bg: "bg-amber-500/10" },
               { name: "Project X", items: 15, color: "text-pink-500", bg: "bg-pink-500/10" },
            ].map((folder, i) => (
               <motion.div 
                  key={i}
               ...
               >
                  <Card className="p-4 border-none shadow-sm hover:shadow-md transition-all cursor-pointer group">
                     ...
                     <h4 className="font-bold text-foreground group-hover:text-primary transition-colors">{folder.name}</h4>
                     <p className="text-xs text-muted-foreground mt-1">{folder.items} files</p>
                  </Card>
               </motion.div>
            ))}
         </div>
         ...
         <div className="bg-card rounded-xl border border-border/50 overflow-hidden shadow-sm">
            <table className="w-full text-sm text-left">
               ...
               <tbody className="divide-y divide-border/50">
                  {[
                     { name: "Employment_Contract.pdf", type: "pdf", date: "Jan 12, 2024", size: "2.4 MB", icon: FileText, color: "text-red-500" },
                     { name: "Q4_Performance_Review.docx", type: "doc", date: "Dec 20, 2023", size: "1.0 MB", icon: FileText, color: "text-blue-500" },
                     { name: "Team_Lunch_Receipt.jpg", type: "img", date: "Feb 02, 2024", size: "4.5 MB", icon: ImageIcon, color: "text-purple-500" },
                     { name: "Salary_Projection_2024.xlsx", type: "sheet", date: "Jan 05, 2024", size: "1.2 MB", icon: FileSpreadsheet, color: "text-emerald-500" },
                     { name: "Onboarding_Checklist.pdf", type: "pdf", date: "Jan 02, 2024", size: "0.5 MB", icon: FileText, color: "text-red-500" },
                  ].map((file, i) => (
                     <tr key={i} className="hover:bg-muted/30 transition-colors group">
                        ...
                        <td className="px-6 py-4 tabular-nums text-muted-foreground">{file.size}</td>
