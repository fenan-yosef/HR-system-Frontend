...
export default function LeaveRequestsPage() {
  return (
    <section className="space-y-8">
            ...
            <Card className="p-6 border-none shadow-sm relative overflow-hidden group">
               ...
               <div className="relative z-10">
                  ...
                  <div className="mt-2 flex items-baseline gap-1">
                     <span className="text-4xl font-black text-foreground">{item.total - item.used}</span>
                  ...
                  <p className="text-xs font-medium text-muted-foreground mt-2">{item.used} days used</p>
               </div>
            </Card>
          </motion.div>
        ))}
      ...
      {/* Requests List */}
      <div>
         <h3 className="text-xl font-bold mb-4 tracking-tight">Recent Requests</h3>
         <div className="grid gap-4">
             {[
                { type: "Annual Leave", from: "Feb 12", to: "Feb 15", days: 3, status: "Approved", statusColor: "text-emerald-500 bg-emerald-500/10", icon: CheckCircle2 },
                { type: "Sick Leave", from: "Jan 10", to: "Jan 10", days: 1, status: "Approved", statusColor: "text-emerald-500 bg-emerald-500/10", icon: CheckCircle2 },
                { type: "Unpaid Leave", from: "Dec 24", to: "Dec 26", days: 2, status: "Rejected", statusColor: "text-red-500 bg-red-500/10", icon: XCircle },
                { type: "Annual Leave", from: "Mar 01", to: "Mar 05", days: 5, status: "Pending", statusColor: "text-amber-500 bg-amber-500/10", icon: Clock },
             ].map((req, i) => (
                <motion.div
                   key={i}
                ...
                >
                   <Card className="p-4 flex items-center justify-between border-l-4 border-l-primary/0 hover:border-l-primary transition-all hover:shadow-md">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${req.statusColor}`}>
