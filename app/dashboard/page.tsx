    ...
    async function load() {
      try {
        const [emp, jobs, apps, leaves] = await Promise.all([
          apiFetch<{ count: number }>("/employees/", { requiresAuth: true }).catch(() => ({ count: 0 })),
          apiFetch<{ count: number }>("/job-posts/", { requiresAuth: true }).catch(() => ({ count: 0 })),
          apiFetch<{ count: number }>("/applications/", { requiresAuth: true }).catch(() => ({ count: 0 })),
          apiFetch<{ count: number }>("/leave-requests/?status=pending", { requiresAuth: true }).catch(() => ({ count: 0 })),
        ]);
        if (mounted) {
          setMetrics({
      ...
      <section className="space-y-6">
        ...
        <div className="rounded-xl border bg-white p-4 shadow">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold">Recent system activities</h2>
              <p className="text-sm text-muted-foreground">Latest changes across recruitment and employees.</p>
            </div>
            <ActionButton role="ADMIN" className="px-3 py-2 text-sm">View details</ActionButton>
          </div>
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            <li>• Placeholder activity feed. Connect to audit logs API.</li>
