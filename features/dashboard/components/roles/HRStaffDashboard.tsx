        ...
        <Card className="p-6 border-none bg-card/50 backdrop-blur-sm shadow-xl md:col-span-2">
          ...
          <div className="space-y-4">
            {metrics.recent_applications.map((app: any, i: number) => (
              <div
                key={i}
                className="group flex items-center justify-between p-4 rounded-2xl bg-muted/20 hover:bg-muted/40 transition-all border border-transparent hover:border-border/50"
              >
                <div className="flex items-center gap-4">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                {
                  label: "Leaves",
                  value: metrics.pending_requests.leaves,
                  icon: Calendar,
                },
                {
                  label: "Letters",
                  value: metrics.pending_requests.letters,
                  icon: FileText,
                },
                {
                  label: "Transfers",
                  value: metrics.pending_requests.transfers,
                  icon: Share2,
                },
              ].map((req, i) => (
                <div
                  key={i}
                  className="p-4 rounded-2xl bg-white/10 backdrop-blur-md flex flex-col items-center gap-2"
                >
        ...
        <Card className="p-6 border-none bg-card/50 backdrop-blur-sm shadow-xl">
          ...
          <div className="space-y-3">
            {metrics.today_tasks.map((task: any, i: number) => (
              <div
                key={i}
                className="flex gap-4 p-4 rounded-2xl border border-border/30 bg-muted/10 hover:bg-muted/20 transition-all cursor-pointer"
              >
                <div
                  className={cn(
